...
export default function Projects() {
  ...
  const items = [
    { icon: "🌐", img: "/images/install-rack.jpg", titleKey: "proj_network", descKey: "proj_network_desc" },
    { icon: "🖥️", img: "/images/network-server.jpg", titleKey: "proj_server", descKey: "proj_server_desc" },
    { icon: "📹", img: "/images/work-camera.jpg", titleKey: "proj_camera", descKey: "proj_camera_desc" },
    { icon: "⏱️", img: "/images/Attendance.jpg", titleKey: "proj_attendance", descKey: "proj_attendance_desc" },
    { icon: "📞", img: "/images/voip-system.jpg", titleKey: "proj_voip", descKey: "proj_voip_desc" },
    { icon: "🛡️", img: "/images/cyber-security.jpg", titleKey: "proj_cyber", descKey: "proj_cyber_desc" },
    { icon: "💻", img: "/images/Web-App.jpg", titleKey: "proj_web", descKey: "proj_web_desc" },
    { icon: "🔧", img: "/images/maintenance.jpg", titleKey: "proj_maintenance", descKey: "proj_maintenance_desc" },
  ];

  return (
    <section
      id="projects"
      className="py-20 px-4 sm:px-6 text-white relative"
      style={{ backgroundColor: "#0049BF" }}
    >
      ...
      <div className="relative max-w-6xl mx-auto">
        {/* الخط المركزي */}
        <div className="hidden md:block absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-[3px] bg-white/20 z-0"></div>
        ...
        {items.map((item, i) => {
          ...
          return (
            <motion.div
              ...
              transition={{ duration: 0.6, delay: i * 0.2 }}
              viewport={{ once: true }}
              className={`relative mb-16 flex flex-col md:flex-row items-center gap-6 sm:gap-10 z-10 ${
                isLeft ? "" : "md:flex-row-reverse"
              }`}
            >
              {/* رقم Timeline */}
              <div className="md:absolute md:left-1/2 md:-translate-x-1/2 flex items-center justify-center w-10 h-10 bg-white text-[#0049BF] font-bold rounded-full border-4 border-[#0049BF] z-20 text-lg mb-4 md:mb-0">
                {i + 1}
              </div>
              ...
              <motion.div
                ...
                transition={{ duration: 0.6, delay: i * 0.2 }}
              >
                <div className="w-full max-w-[320px] h-44 sm:h-52 rounded-2xl overflow-hidden shadow-xl group bg-white/10 border border-white/10 cursor-pointer">
                  <img
                    src={item.img}
                    alt={t(item.titleKey)}
                    className="w-full h-full object-cover transition duration-700 group-hover:scale-110"
              ...
              {/* النص */}
              <motion.div
                className={`w-full md:w-1/2 text-center md:text-${isLeft ? "left" : "right"}`}
                initial={{ opacity: 0, x: isLeft ? 50 : -50 }}
                whileInView={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.6, delay: i * 0.3 }}
              >
                <div className="text-3xl sm:text-4xl mb-2">{item.icon}</div>
                <h3 className="text-xl sm:text-2xl font-bold mb-2">{t(item.titleKey)}</h3>
